...
### 1.10 `synora config gate-set (--enable|--disable) [--confirm] [--approval-record <ref>] [--gate-version <version>] [--reason <text>] [--keep-record] [--dry-run] [--json]` (Phase 3 utility)

Purpose:
- Write execution gate config (`execution.*`) in a controlled CLI path.

### 1.11 `synora source suggest [--json] [--verbose]` (Phase 2 Week 2 prototype)
