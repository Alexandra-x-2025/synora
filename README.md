...
2. 前台提供“快捷操作”按钮（更新检测、发现、仓库同步、AI 分析/建议/修复方案），点击后由后端 API 执行。
3. 结果卡片直接点击 `执行`，高风险动作会弹确认框。
4. 设置项（安全策略、调度、worker 等）放在 `设置` 面板，默认不打扰主流程。
5. 支持 `中文 / EN` 一键切换（localStorage 持久化）。
6. 前端不展示命令行字符串，所有操作均通过后端 API 完成。
7. JSON 手动渲染能力在“高级（开发调试）”折叠区。
